...
import cc.abro.orchengine.net.server.senders.ServerSendUDP;
...
public class NetServerRead implements NetServerReadInterface {
    ...
    public void readTCP(MessagePack.Message message) {
        ...
        switch (message.type) {
            case 1:
                take1(message);
                break;//Engine: Клиент пингует сервер
            case 6:
                take6(message);
                break;
            case 12:
    ...
    public void take16(MessagePack.Message message) {
        ...
        if (ServerData.playerData[answerDataId].color == null || ServerData.playerData[answerDataId].name == null)
            return;
